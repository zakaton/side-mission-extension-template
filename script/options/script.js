...
sideMission.addEventListener("euler", event => {
    ...
    let pitch = euler.x
    pitch = THREE.Math.clamp(pitch, pitchMin, pitchMax)
    slouch = inverseLerp(pitchMax, pitchMin, pitch)
...
let pitchMin = -Math.PI
document.getElementById("pitchMin").addEventListener("click", event => {
    if (sideMission.euler.x < pitchMax) {
        pitchMin = sideMission.euler.x
        samples.euler.pitchMin.fill(pitchMin)
        draw("euler")
    }
})

let pitchMax = Math.PI
document.getElementById("pitchMax").addEventListener("click", event => {
    if (sideMission.euler.x > pitchMin) {
        pitchMax = sideMission.euler.x
        samples.euler.pitchMax.fill(pitchMax)
        draw("euler")
    }
})