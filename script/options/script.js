...
sideMission.addEventListener("quaternion", event => {
    ...
    callibratedEuler.setFromQuaternion(callibratedQuaternion)
            ...
            tabs.forEach(tab => {
                if (tab.id !== currentTab.id) {
                    chrome.tabs.sendMessage(tab.id, {
                        ukaton: "sideMission",
                        type: "callibratedEuler",
                        value: callibratedEuler.toArray()
                    })
                }
            })
        }
...
const callibratedEuler = new THREE.Euler(0,0,0, "YXZ")
